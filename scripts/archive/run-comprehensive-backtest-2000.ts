...
async function runComprehensiveBacktest() {
    const groundTruthPath = path.join(process.cwd(), 'lib/backtest/ground-truth-2000.json');

    if (!fs.existsSync(groundTruthPath)) {
        console.error('❌ ground-truth-2000.json not found. Please run generate-ground-truth-2000.ts first.');
        process.exit(1);
    }
    ...
    const outputPath = path.join(process.cwd(), 'lib/backtest/results-2000.json');
