...
// If wind is within these ranges, the wind risk score is reduced.
const SAFE_WIND_DIRECTIONS: Record<string, number[][]> = {
    'jr-hokkaido.chitose': [[350, 360], [0, 10]], // North wind (Parallel to line) is safe
};
...
async function fetchWeather(lat: number, lon: number, date: string) {
    ...
    try {
        ...
        const dir = data.hourly.winddirection_10m;
        ...
        // Use max values for the day to represent "worst case" for that day
        // For direction, we should pick the direction AT the time of max wind speed to be accurate.
        // Let's find index of max wind.
        const maxWindVal = Math.max(...wind);
        const maxWindIdx = wind.indexOf(maxWindVal);
        const windDirAtMax = dir[maxWindIdx] || 0;

        return {
            wind_speed: maxWindVal,
            wind_direction: windDirAtMax,
...
function determineStatusAndRecovery(features: any, routeId: string) {
    const { wind_speed, wind_direction, wind_gust, snowfall, snow_depth, temperature } = features;
    ...
    // Wind Score Calculation with Direction Logic
    let windScore = Math.max(0, wind_speed - 15) * 5;

    // Check Direction
    if (isSafeWind(wind_direction, SAFE_WIND_DIRECTIONS[routeId])) {
        windScore *= 0.4; // 60% reduction if direction is safe
    }
...
async function generate() {
    const outPath = path.join(process.cwd(), 'lib/backtest/dataset_5k.csv');
    const stream = fs.createWriteStream(outPath);

    // Added wind_direction
    stream.write('route_id,date,month,wind_speed,wind_direction,wind_gust,snowfall,snow_depth,temperature,pressure,status,recovery_time\n');
    ...
    // Shared counter managed by atomic ++ in single thread loop
    async function worker() {
        while (generated < TARGET) {
            ...
            if (weather) {
                ...
                // Added weather.wind_direction
                stream.write(`${routeId},${date},${weather.month},${weather.wind_speed},${weather.wind_direction},${weather.wind_gust},${weather.snowfall},${weather.snow_depth},${weather.temperature},${weather.pressure},${status},${recovery_time}\n`);
